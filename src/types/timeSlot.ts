 ...
 */
export function createHourlyConfig(
  profileId: string,
  loadLevel: LoadLevel,
): HourlyConfig {
...
export function createEmptySchedule(): WeeklySchedule {
  const emptyDay = (): HourlySchedule =>
    Array(24)
      .fill(null)
      .map(() => createBaselineConfig());
  return {
    mon: emptyDay(),
    tue: emptyDay(),
    wed: emptyDay(),
 ...
 */
export function createFilledSchedule(
  profileId: string,
  loadLevel: LoadLevel,
): WeeklySchedule {
  const filledDay = (): HourlySchedule =>
    Array(24)
      .fill(null)
      .map(() => createHourlyConfig(profileId, loadLevel));
  return {
    mon: filledDay(),
    tue: filledDay(),
    wed: filledDay(),
