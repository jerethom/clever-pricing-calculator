import type { AddonProvider, Instance, Zone } from "./types";
...
async function fetchRuntimePrices(): Promise<Map<string, number>> {
	const response = await fetch(
		`${API_BASE_URL}/v4/billing/price-system?zone_id=par`,
	);

	if (!response.ok) {
		console.warn(
			"Impossible de récupérer les prix v4, utilisation des prix par défaut",
		);
		return new Map();
	}
...
export async function fetchInstances(): Promise<Instance[]> {
	...
	if (!instancesResponse.ok) {
		throw new Error(
			`Erreur lors de la récupération des instances: ${instancesResponse.status}`,
		);
	}
...
export async function fetchAddonProviders(): Promise<AddonProvider[]> {
	...
	if (!response.ok) {
		throw new Error(
			`Erreur lors de la récupération des addons: ${response.status}`,
		);
	}
...
export async function fetchZones(): Promise<Zone[]> {
	...
	if (!response.ok) {
		throw new Error(
			`Erreur lors de la récupération des zones: ${response.status}`,
		);
	}
