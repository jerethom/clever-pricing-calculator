...
export const OrganizationBudgetGauge = memo(function OrganizationBudgetGauge({
  ...
  const [editValue, setEditValue] = useState(budgetTarget ?? Math.ceil(currentCost * 1.2))
  ...
  const presets = useMemo((): BudgetPreset[] => [
    ...
    { label: '+20%', value: roundBudget(currentCost * 1.2), isRecommended: true },
  ...
  const budgetPercent = budgetTarget && budgetTarget > 0
    ? (currentCost / budgetTarget) * 100
    : 0
    ...
    >
      <div className="card-body p-4 sm:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
          ...
          <div className="flex-1">
            {isEditing ? (
              showCustomInput ? renderCustomInput() : renderPresets()
            ) : budgetTarget ? (
              renderBudgetStatus()
            ) : (
              renderNoBudget()
