...
export function RuntimeForm({ projectId, onClose }: RuntimeFormProps) {
  ...
  const selectedFlavorData = selectedInstance?.flavors.find(f => f.name === selectedFlavor)
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="mt-4 text-base-content/60">Chargement des runtimes...</p>
            </div>
          ) : (
            <>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
                          {group.map(instance => (
                            <RuntimeCard
                              ...
                              instance={instance}
                              isSelected={selectedVariantId === instance.variant.id}
                              onClick={() => handleVariantSelect(instance.variant.id)}
                            />
                          ))}
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-4 p-4 bg-base-200 border border-base-300">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold truncate">{selectedInstance.name}</h3>
                      ...
                      {selectedInstance.flavors
                        ...
                        .map(flavor => (
                          <FlavorCard
                            ...
                            isDefault={selectedInstance.defaultFlavor.name === flavor.name}
                            maxCpus={Math.max(...selectedInstance.flavors.map(f => f.cpus))}
                  ...
                  <div className="flex items-center gap-4 p-4 bg-base-200 border border-base-300">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-base-content/60">
                        {selectedFlavor} - {selectedFlavorData.memory.formatted} / {selectedFlavorData.cpus} vCPU
                      </p>
                    </div>
                    <button
                      className="btn btn-ghost btn-sm"
...
function RuntimeCard({ instance, isSelected, onClick }: RuntimeCardProps) {
  return (
    ...
    >
      ...
      <span className="text-xs sm:text-sm font-medium truncate w-full">{instance.name}</span>
      <span className="text-xs text-base-content/50 truncate w-full">{instance.type}</span>
    </button>
  )
}
...
function FlavorCard({ flavor, isSelected, isDefault, maxCpus, onClick }: FlavorCardProps) {
