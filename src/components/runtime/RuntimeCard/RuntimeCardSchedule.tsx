...
export const RuntimeCardSchedule = memo(function RuntimeCardSchedule({
  ...
  const { runtime, cost, showTimeSlots, onToggleTimeSlots } = useRuntimeCardContext()
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex gap-0.5">
            {DAYS_OF_WEEK.map((dayKey, i) => {
              const hasScaling = runtime.weeklySchedule?.[dayKey]?.some(h => h.loadLevel > 0) ?? false
          ...
          <span className={`badge badge-sm ${showTimeSlots ? 'badge-ghost' : ''}`}>
